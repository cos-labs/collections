...
/**
  `DS.hashMany` is used to define One-To-Many and Many-To-Many
  relationships on a [DS.Model](/api/data/classes/DS.Model.html) through a hash rather
  than array, allowing a compact representation of some simple through tables.
  `DS.hashMany` takes an optional hash as a second parameter, currently
...
export default function hasMany(type, options) {
  ...
  assert(`The first argument to DS.hashMany must be a string representing a model type key, not an instance of ${Ember.inspect(type)}. E.g., to define a relation to the Comment model, use DS.hashMany('comment')`, typeof type === 'string' || typeof type === 'undefined');
